...
<table class="table table-bordered w-100 mb-3" cellspacing="0">
  ...
  <tbody>
    ...
    <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-light' : '' }}">
      ...
      <td class="align-middle" rowspan="{{ count($transaction->purchases) }}">
    {{ $transaction->user ? $transaction->user->name : 'N/A' }}
</td>
      <td class="align-middle">@if ($transaction->purchases->isNotEmpty() && $transaction->purchases[0]->product)
        {{ $transaction->purchases[0]->product->name }}
        @else
        Product not found
        @endif
      </td>
      <td class="align-middle">@if ($transaction->purchases->isNotEmpty() && $transaction->purchases[0]->product && isset($transaction->purchases[0]->product->price))

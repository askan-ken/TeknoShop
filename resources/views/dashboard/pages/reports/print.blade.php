...
<body>
  <div style="width: 10%; float:left; margin-right: 20px;">
    <img src="{{ url('assets/images/logo-2.png') }}" alt="" height="75px">
  </div>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-light' : '' }}">
        ...
        <td rowspan="{{ $transaction->purchases ? count($transaction->purchases) : 1 }}">
    {{ $transaction->user ? $transaction->user->name : 'N/A' }}
</td>
        <td>@if ($transaction->purchases->isNotEmpty() && $transaction->purchases[0]->product)
          {{ $transaction->purchases[0]->product->name }}
          @else
          Product not found
          @endif
        </td>
        <td>@if ($transaction->purchases->isNotEmpty() && $transaction->purchases[0]->product && isset($transaction->purchases[0]->product->price))

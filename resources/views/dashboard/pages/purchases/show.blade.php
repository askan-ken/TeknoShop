            ...
            <div class="card border-0 shadow-none">
              <div class="row g-0">
                <div class="col-5 col-md-4 col-lg-2">
                  @if ($purchase->product && $purchase->product->photo)
                  <img src="/storage/{{ $purchase->product->photo }}" class="img-fluid rounded" alt="{{ $purchase->product->name }}">
                  @else
                  <!-- Handle case where product or photo is not available -->
                ...
                <div class="col-7 col-md-8 col-lg-10">
                  <div class="card-body py-1">
                    <p class="card-text fw-bold m-0">@if ($transaction->purchases->isNotEmpty() && $transaction->purchases[0]->product)
                      {{ $transaction->purchases[0]->product->name }}
                      @else
                      Product not found
                      @endif
                    </p>
                    <p class="card-text my-text-gray m-0">@if ($transaction->purchases->isNotEmpty() && $transaction->purchases[0]->product && isset($transaction->purchases[0]->product->price))
                      {{ $transaction->purchases[0]->product->price }}
                      @else
                      Price not available
                      @endif
                    </p>
